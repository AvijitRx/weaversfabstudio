        ...
        <div class="max-md:max-y-6 flex justify-center gap-6 max-lg:flex-wrap max-md:grid max-md:grid-cols-2 max-md:gap-x-2.5 max-md:text-center">
            @foreach ($customization->options['services'] as $service)
                <div class="flex items-center gap-5 max-md:grid max-md:gap-2.5 max-sm:gap-1 max-sm:px-2">
                    <span
                        class="{{ $service['service_icon'] }} flex h-[60px] w-[60px] items-center justify-center rounded-full bg-paper p-2.5 text-4xl text-madder max-md:m-auto max-md:h-16 max-md:w-16 max-sm:h-10 max-sm:w-10 max-sm:text-2xl"
                        role="presentation"
                    >
                    </span>
                    ...
                    <div class="max-lg:grid max-lg:justify-center">
                        <!-- Service Title -->
                        <p class="font-dmserif text-lg max-md:text-xl max-sm:text-sm">
                            {{ $service['title'] }}
                        </p>

                        <!-- Service Description -->
                        <p class="mt-1.5 max-w-[217px] text-sm text-inkSoft max-md:mt-0 max-md:text-base max-sm:text-xs">
                            {{ $service['description'] }}
                        </p>
                    </div>
                </div>
